...
%macro		startTimer 0
                mov     ax, 03508h
                int     21h
                mov     word [Old08Irqseg], es
                mov     word [Old08Irqofs], bx
                mov     ax, 02508h
                mov     dx, irqHandler08
                int     21h
...
%macro		startKBDHandler 0
                ;mov     ax, 03509h
                ;int     21h
                ;mov     word [Old09Irqseg], es
                ;mov     word [Old09Irqofs], bx
                ;mov     ax, 02509h
                ;mov     dx, irqHandler09
                ;int     21h
...
cF1loop:
		mov	bx, 4
		call	rnd
		add	al, byte [es:di-64]
		add	al, byte [es:di-63]
		dec	ax
		shr	ax, 1
		stosb
		loop	cF1loop
...
%macro		colorCycle 0
		...
		mov	si, di
		lodsw
		;mov	bl, byte [si]  ; works only with fire (no blue)
		inc	si
		mov	cx, (range*3-3)
		rep movsb
		stosw
		;mov	byte [di], bl  ; works only with fire (no blue)
%endmacro
...
EPSwhile:	lodsw
                ...
                mov	byte [bx+channel.trk], al ; [bx+2]
                add	al, CMD_CHANGEPRG ; 0C0h
                call	setinstr
                mov	word [bx+channel.adr], di ; [bx+4]	
               	call	expand
                xor	al, al
                stosb
                add	bx, 6
...
sImloop:
		...
		mov	bx, word [si+8]	      ;	scriptptr
		;mov	bx, word [scriptptr]
		mov	al, byte [bx+script]
		or	al, al
		jz	_exit_
		mov	bx, ax
		and	bl, 7
		mov	byte [si+12], bl      ;	scriptins
		;mov	byte [scriptins], bl
		dec	bl
...
sInegateTurn:
		and	ax, 0F8h
		add	ax, ax
		mov	word [si+10], ax      ; scriptanz
		;mov	word [scriptanz], ax
		inc	word [si+8]	      ; scriptptr
		;inc	word [scriptptr]
sIwaitTimer:
		mov	al, byte [si+12]      ;	scriptins
		;mov	al, byte [scriptins]
		cbw
		;xor	ch, ch
		dec	ax
		jnz	short sInoturn
...
sFiloop:
		...
		inc	si
		loop	sFmloop
		push	di	; facei+4*(o+object.fanz-1)
		call	qsort
%endmacro
...
PSmloop:
		...
		jg	short PSdelayed
		mov	ax, word [si+channel.trk]
		add	al, CMD_NOTEOFF	; 80h
		call	setnote
		mov	bx, word [si+channel.adr]
		mov	ah, 127 	; 7Fh
		cmp	byte [bx], 0
		jz	short PStrackend
		mov	ah, byte [bx]
		mov	byte [si+channel.ln], ah
		mov	al, byte [si+channel.trk]
...
drawloop:
		lodsw
		lodsw
		mov	bx, ax ; face address in 'o+object.f'
		pusha
...
scansubtextpoly:
		...
		pol	equ 8  ; arg 3, [sp+6] ; dcf.p2 address
 		...
 		enter	150, 0	; 96h
		;enter	152, 0  ; 98h		
			; push bp ; mov bp, sp ; sub sp, 152

		; bp - x
		sstp.edg  equ -60   ; 90-150 ; edges * 3
		sstp.del  equ -120  ; 30-150 ; edges * 3 
		sstp.mx1  equ -122  ; 28-150 ; word
		sstp.mx2  equ -124  ; 26-150 ; word	 				
		sstp.miny equ -126  ; 24-150 ; word
		sstp.maxy equ -128  ; 22-150 ; word	
		sstp.y	  equ -130  ; 20-150 ; word
		sstp.l	  equ -132  ; 18-150 ; word	 				
		sstp.r	  equ -134  ; 16-150 ; word
		sstp.uu1  equ -136  ; 14-150 ; word
		sstp.vv1  equ -138  ; 12-150 ; word
		sstp.uu2  equ -140  ; 10-150 ; word	 				
		sstp.vv2  equ -142  ;  8-150 ; word
		sstp.ddu  equ -144  ;  6-150 ; word
		sstp.ddv  equ -146  ;  4-150 ; word
		sstp.dy   equ -150  ;  0-150 ; dword
		; 
		;sstp.pad  equ -152  ; padding (for dword boundary)
		...
		; di = dcf.p2 address
		mov	di, word [bp+pol] ; [bp+8] ; dcf.p2 ; face structure

		xor	cx, cx
minmax:
		cmp	ax, word [di+2+edge.e+edges.x] ; [di+12h]
		jle	short noswap1
		mov	ax, word [di+2+edge.e+edges.x] ; [di+12h]
noswap1:
		cmp	bx, word [di+2+edge.e+edges.x] ; [di+12h]
		jge	short noswap2
		mov	bx, word [di+2+edge.e+edges.x] ; [di+12h]
noswap2:
		cmp	dx, word [di+edge.py]  ; [di+0Ch]
		jle	short noswap3
		mov	dx, word [di+edge.py]  ; [di+0Ch]
		mov	word [bp+sstp.l], cx   ; [bp-84h]
		mov	word [bp+sstp.r], cx   ; [bp-86h]
noswap3:
		cmp	si, word [di+edge.py]  ; [di+0Ch]
		jge	short noswap4
		mov	si, word [di+edge.py]  ; [di+0Ch]
noswap4:
		add	di, 36 ; 24h
		inc	cx
		cmp	cx, word [bp+n] ; [bp+6]
		...
		cmp	dx, YMAX  ; 0B3h
		jg	exitDraw
		mov	word [bp+sstp.miny], dx ; [bp-7Eh]
		mov	word [bp+sstp.maxy], si ; [bp-80h]

		;{ down clipping }
		cmp	word [bp+sstp.maxy], YMAX ; 0B3h
		jle	short noclipdown
		mov	word [bp+sstp.maxy], YMAX ; 0B3h
noclipdown:
		mov	ax, word [bp+sstp.miny] ; [bp-7Eh]
		cmp	ax, word [bp+sstp.maxy] ; [bp-80h]
		je	exitDraw
		mov	word [bp+sstp.y], ax    ; [bp-82h]
		mov	bx, word [bp+col]	; [bp+4]
					   ; dcf.col ; shade/color value
		add	bx, bx
		mov	es, word [vseg]
		mov	fs, word [bx+tseg]
for:
		mov	ax, word [bp+sstp.y]    ; [bp-82h]
		cmp	ax, word [bp+sstp.maxy] ; [bp-80h]
		jg	exitDraw
		;je	exitwhile2
while1:
		...
		mov	si, word [bp+pol]	; [bp+8]
		imul	bx, word [bp+sstp.l], 36 ; [bp-84h]
		add	si, bx
		;mov	si, bx
		;add	si, word [bp+pol]	; [bp+8]

		cmp	ax, word [si+edge.py]	; [si+0Ch]
		jne	short while2

		mov	bx, word [bp+sstp.l]	; [bp-84h]
		dec	bx
		jge	short nounder
		mov	bx, word [bp+n]		; [bp+6]
					   ; dcf.j ; visible edges/points
		dec	bx
nounder:
		mov	word [bp+sstp.l], bx	; [bp-84h]
		xor	di, di
		call	calcDeltas
		;mov	ax, word [bp+sstp.y]    ; [bp-82h]
		;cmp	ax, word [bp+sstp.maxy] ; [bp-80h]
...
while2:
		...
		jge	short exitwhile2
		mov	si, word [bp+pol]	; [bp+8]
		imul	bx, word [bp+sstp.r], 36 ; [bp-86h]
		add	si, bx
		;
		;mov	si, bx
		;add	si, word [bp+pol]	; [bp+8]
		...
		mov	bx, word [bp+sstp.r]    ; [bp-86h]
		inc	bx
		cmp	bx, word [bp+n]		; [bp+6]
		jl	short noover
		xor	bx, bx
noover:
		mov	word [bp+sstp.r], bx	; [bp-86h]
		mov	di, 20  ; 14h
		call	calcDeltas
		;
		;mov	ax, word [bp+sstp.y]	; [bp-82h]
...
exitwhile2:
		;{ up clipping }
		cmp	word [bp+sstp.y], YMIN	; 15h
		jl	clipup
		xor	ebx, ebx
		xor	ecx, ecx
		mov	bx, word [bp+sstp.edg+2+edges.x] ; [bp-3Ah]
		mov	word [bp+sstp.mx1], bx  ; [bp-7Ah]
		mov	cx, word [bp+sstp.edg+20+2+edges.x] ; [bp-26h]
		mov	word [bp+sstp.mx2], cx  ; [bp-7Ch]
		sub	cx, bx
		jz	exitol
		cmp	word [bp+sstp.mx2], 0 ; [bp-7Ch]
		jle	exitol
		cmp	word [bp+sstp.mx1], XMAX ; 140h
		jg	exitol
		...
		jmp	short myendif
;_nodivbyzero:
;		mov	eax, dword [bp+di+sstp.del+40] ; [bp+di-50h]
;		jmp	short nodivbyzero
myelse:
		mov	word [bp+sstp.mx1], 0 ; [bp-7Ah]
myendif:
		mov	di, 16 ; 10h
addloop1:
		;{ calculate deltas }
		mov	eax, dword [bp+di+sstp.edg+20] ; [bp+di-28h]
		sub	eax, dword [bp+di+sstp.edg]    ; [bp+di-3Ch]
		cdq
		jcxz	nodivbyzero
		;jcxz	_nodivbyzero
		idiv	ecx
		mov	dword [bp+di+sstp.del+40], eax ; [bp+di-50h]
nodivbyzero:
		;{ left clipping }
		mov	eax, dword [bp+di+sstp.del+40] ; [bp+di-50h]
		imul	ebx
		add	eax, dword [bp+di+sstp.edg]	 ; [bp+di-3Ch]
		mov	dword [bp+di+sstp.edg+40], eax ; [bp+di-14h]
		sub	di, 4
		jnz	short addloop1

		mov	ax, word [bp+sstp.del+40+16]   ; [bp-40h]
		;mov	word [cs:dels_pos+1], ax
		mov	word [dels_pos_w], ax

		;{ right clipping }
		cmp	word  [bp+sstp.mx2], XMAX  ; [bp-7Ch]
		jle	short norightclip
		mov	word  [bp+sstp.mx2], XMAX  ; 140h
norightclip:
		imul	di, word [bp+sstp.y], 320  ; [bp-82h]
		add	di, word [bp+sstp.mx1]	   ; [bp-7Ah]
		
		mov	bx, 256 ; 100h
		mov	eax, dword [bp+sstp.edg+40+edges.u] ; [bp-10h]
		imul	ebx
		idiv	dword [bp+sstp.edg+40+edges.w] ; [bp-8]
		mov	word [bp+sstp.uu2], ax 	    ; [bp-8Ch]
		mov	eax, dword [bp+sstp.edg+40+edges.v] ; [bp-0Ch]
		imul	ebx
		idiv	dword [bp+sstp.edg+40+edges.w] ; [bp-8]
		mov	word [bp+sstp.vv2], ax	    ; [bp-8Eh]
outloop:
		mov	cx, word [bp+sstp.mx2]	    ; [bp-7Ch]
		sub	cx, word [bp+sstp.mx1]	    ; [bp-7Ah]
		jle	exitol
		cmp	cx, SUBRANGE ; 10h
		jle	short lastSeg
		mov	cx, SUBRANGE ; 10h
lastSeg:
		;{ uu1:=uu2 }
		;{ vv1:=vv2 }
		mov	eax, dword [bp+sstp.vv2]    ; [bp-8Eh]
		mov	dword [bp+sstp.vv1], eax    ; [bp-8Ah]

		mov	eax, dword [bp+sstp.del+40+edges.u] ; [bp-4Ch]
		imul	ecx
		add	dword [bp+sstp.edg+40+edges.u], eax ; [bp-10h]
		mov	eax, dword [bp+sstp.del+40+edges.v] ; [bp-48h]
		imul	ecx
		add	dword [bp+sstp.edg+40+edges.v], eax ; [bp-0Ch]
		mov	eax, dword [bp+sstp.del+40+edges.w] ; [bp-44h]
		imul	ecx
		add	dword [bp+sstp.edg+40+edges.w], eax ; [bp-8]
		...
		mov	ax, word [bp+sstp.uu2]	    ; [bp-8Ch]
		sub	ax, word [bp+sstp.uu1]	    ; [bp-88h]
		cwd
		idiv	cx
		;mov	word [cs:ddu_pos+2], ax
		mov	word [ddu_pos_w], ax

		mov	ax, word [bp+sstp.vv2]	    ; [bp-8Eh]
		sub	ax, word [bp+sstp.vv1]	    ; [bp-8Ah]
		cwd
		idiv	cx
		;mov	word [cs:ddv_pos+2], ax
		mov	word [ddv_pos_w], ax

		mov	si, word [bp+sstp.vv1]	    ; [bp-8Ah]
		mov	dx, word [bp+sstp.uu1]	    ; [bp-88h]
		mov	ax, word [bp+sstp.edg+40+16] ; [bp-4]
innerloop:
		mov	bx, si
		xor	bl, bl
		shr	bx, 2
...
dels_pos:	
		add	ax, word [dels_pos_w]
ddu_pos:	
		add	dx, word [ddu_pos_w]
ddv_pos:	
		add	si, word [ddv_pos_w]

		inc	di	
		loop	innerloop

		mov	word [bp+sstp.edg+40+16], ax ; [bp-4]
		add	word [bp+sstp.mx1], SUBRANGE ; [bp-7Ah]
		jmp	outloop
exitol:
clipup:		mov	di, 36 ; 24h
addloop:
		mov	eax, dword [bp+di+sstp.del]  ; [bp+di-78h]
		add	dword [bp+di+sstp.edg], eax  ; [bp+di-3Ch]
		sub	di, 4
		jns	short addloop
		inc	word [bp+sstp.y]	 ; [bp-82h]
		jmp	for
exitDraw:
		leave
			; mov sp, bp ; pop bp
...
calcDeltas:
		...
		;add	si, word [bp+pol]	; [bp+8]	
		mov	ax, word [si+12]
		sub	ax, word [bp+sstp.y] ; [bp-82h]
		movsx	eax, ax
		mov	dword [bp+sstp.dy], eax ; [bp-96h]
		mov	cx, 5
cDloop:
		mov	edx, dword [bx+16]
		mov	dword [bp+di+sstp.edg], edx ; [bp+di-3Ch]
		mov	eax, dword [si+16]
		sub	eax, edx
		cdq
		cmp	word [bp+sstp.dy], 0  ; [bp-96h]
		je	short cDskip
		idiv	dword [bp+sstp.dy]    ; [bp-96h]
		mov	dword [bp+di+sstp.del], eax ; [bp+di-78h]
cDskip:
		add	di, 4
		add	si, 4
		add	bx, 4
...
drawclippedface:
		...
		mov	cx, 4
dCFinitloop:
		push	bx	; face addr (in 'o+object.f')(29/01/2017)
...
intensityOK:
		...
		xor	edx, edx
		shl	ax, 7
		mul	word [bx+o+6+object.p]	; [bx+o+6+4]
		; edges(4-cx).s
...
cWmainLoop:
		...
		and	bx, 7000h
		add	bh, 15 ; 0Fh
		mov	word [bp+cw.shade], bx ; [bp-4]
		and	ax, 0Fh ; 15   ; Low 4 bits for direction
		mov	word [bp+cw.direc], ax ; [bp-2]
...
cWaddPointloop:
		...
		push	di
		mov	di, o+object.p  ; p area of object structure (in o:)
		push	cx
...
cWnoWall:
		...
		imul	di, word [bp+cw.direc], 12 ; [bp-2]
		; di will have negative value if 12*[bp-2] > 127
		; max value of [bp-2] = 0Fh
		mov	cl, 3
...
cWSround:
		...
		inc	si
		loop	cWSround
		mov	ax, word [bp+cw.shade]  ; [bp-4]
		mov	word [si], ax ; 4th word
		popa
		inc	bx
