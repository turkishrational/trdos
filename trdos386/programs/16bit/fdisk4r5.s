; ****************************************************************************
; FDISK4R5.ASM (FDISK4R5.COM) - Retro DOS v5 Hard Disk Partitioning Utility
; fdisk4.s						(for MSDOS/WINDOWS)
; ****************************************************************************
...
; ----------------------------------------------------------------------------
; Modified from 'FDISK4.ASM' (FDISK4.COM) source code by Erdogan Tan
; (04/05/2024) - TRDOS 386 v2 fixed disk partitioning utility -
; ****************************************************************************
; nasm fdisk4r5.s -l fdisk4r5.txt -o FDISK4R5.COM -Z error.txt
...
; Note: 80386 cpu is required (this program will not run on 80286 systems)

	; Detect 80386 cpu (386+)
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
; detect 386+ cpu
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
; 06/11/2020
	; (https://forum.osdev.org/viewtopic.php?p=36822)
	; (ogotay)
	...
	; Write program message and terminate
...
A_23:
	...
	; 05/11/2020
	; fdisk4 can be used for 2TB hard disks
	;	(because of 32 bit virtual cylinder number)
 	; (fdisk3 limit is 502GB disks size and 65535 cylinders)
...
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	; 05/11/2020 
	; (fdisk4 is proper for -upto- 2TB hard disks)
;A_27:
	;; 16/10/2020
	;; 15/10/2020
...
A_32:
	...
	;cmp	di, [bx+part_table_rel_sec+2]
	;jne	short A_34 ; Invalid !

	;;cmp	si, [part_table_rel_sec_lw+bx]
	;cmp	si, [bx+part_table_rel_sec]
	;jne	short A_34 ; Invalid !
		; di:si = partition's start sector (LBA)

	cmp	eax, [part_table_rel_sec+bx]
	jne	short A_34 ; Invalid !
	...
	; 10/03/2021
	mov	eax, [part_table_num_sec+bx]
	add	eax, [part_table_rel_sec+bx]
...
A_33:
	...
	; 05/11/2020
	add	bx, 22 ; partition table structure size	

	jmp	A_32
A_34:
	;mov	si, msg_defective_pt
	; 10/03/2021
	mov	esi, msg_defective_pt
	call	print_msg
	jmp	short A_38
...
A_36:
	...
	; 10/03/2021
	mov	eax, [part_table_end_cyl+di]
	cmp	eax, [part_table_start_cyl+si]
	jb	short A_37
	ja	short A_34 ; overlap !
	...
	;	 ; dx:ax = end sector + 1
	; 10/03/2021
	mov	eax, [part_table_rel_sec+di] ; previous
	add	eax, [part_table_num_sec+di]
	...
	; 10/03/2021
	cmp	eax, [part_table_rel_sec+si] ; current
	jnb	short A_34 ; overlap error !
 A_37:
	cmp	bl, 3
	jb	short A_36
...
chk_dp_1:
	...
	retn
chk_dp_2:
	;cmp	bx, part_table_boot_ind+(22*4) ; 05/11/2020
	cmp	si, 3
	jnb	short chk_dp_3
	;add	bx, 18
	; 05/11/2020
...
;cp_0:
	...
	; 12/03/2021
	mov	eax, [free_space.sectors_unused+bx]
...
wptmbr_1:
	...
	mov	di, MasterBootBuff+419
	
	; (if disk size > 512 GB, number of cylinders will be > 65535)
	mov	al, [cylinders+2]
	stosb
	;
	mov	ax, [cylinders]
...
cpp_0:
	...
	;mov	al, [heads]
	;; 07/11/2020
	;movzx	eax, byte [heads]
	;mul	byte [sectors]
	;;mul	bx
	;	; DX:AX = LBA of start cylinder, head 0, sector 1
	; 14/03/2021
	mov	eax, [hs] ; [heads] * [sectors]
	; 07/11/2020
	mov	ebx, [di+free_space.start]
	mul	ebx
	;	; EAX =  LBA of start cylinder, head 0, sector 1
	...
	; 07/11/2020
	mov	eax, [di+free_space.startsector]
	jmp	short cpp_4
cpp_1:
	;cmp	cl, 27 ; ESCape
	;jne	short cpp_4 ; 'Y'
	...
	mov	ecx, [di+free_space.startsector]
	or	ebx, ebx
	jnz	short cpp_2 ; start cylinder > 0
...
cpp_5:
	...
	; 14/03/2021
	or	eax, ecx
	jz	short cpp_24 ; cyl = 0, sector = 0
...
cpp_7:
	...
	; 14/03/2021
	; eax = start sector address (LBA)

	; 07/11/2020
	sub	edx, edx ; 14/03/2021
	;movzx	ecx, byte [sectors]
	xor	ecx, ecx
	mov	cl, [sectors]
	div	ecx
...
cpp_18:
	...
	;; ax = heads*sectors
	; 14/03/2021
	;mov	eax, [hs] ; heads * spt

	;mov	bx, [di+free_space.end]	; end cylinder of the partition
	; 14/03/2021
	;mov	ebx, [di+free_space.end]
	mov	eax, [di+free_space.end]
	...
	;mov	ebx, eax
	mov	eax, [ppn_Sectors]
	add	eax, [pp_StartSector]
	dec	eax
	xor	edx, edx
	;div	ebx
	div	dword [hs] ; heads * spt
		; eax = (end) cylinder number
...
cpp_9:
	...
	; 14/03/2021
	cmp	eax, [di+free_space.end]
	jna	short cpp_10 ; ok
	dec	eax
cpp_10:
	push	eax ; * ; end cylinder
	;mul	bx
	;	 ; dx:ax = (end cylinder)*heads*sectors
	; 14/03/2021
	mul	dword [hs] ; heads * spt
		; eax = (end cylinder)*heads*sectors
		; edx = 0
	pop	edx ; * ; end cylinder
	...
	;mov	cx, 3FFFh
	; 14/03/2021
	cmp	edx, 1023  ; CHS limit
	jna	short cpp_19
	; > CHS limit
	mov	edx, 1023 ; cylinder
	mov	ax, 0FF3Fh ; heads = 255, sector = 63
cpp_19:
	;dec	cl ; heads - 1 = end head
	; 14/03/2021
	dec	ah ; heads - 1 = end head

	; 14/03/2021
	; al = sectors (spt), ah = end head
	...
	shl	dh, 6
	or	dh, al ; or dh, byte [sectors]
	mov	[si+ptEndSector], dh
	...
	; 14/03/2021
	add	eax, edx ; ([end head]*sectors)+sectors-1
	pop	edx ; (end cylinder)*heads*sectors
	add	eax, edx
	; eax = (([end cylinder]*heads)+[end head])*sectors)+sectors-1
	...
	mov	ecx, eax
	inc	ecx
	sub	ecx, [si+ptStartSector]
		; ecx = partition size in sectors
...
cpp_12:
	...
	;sbb	dx, 0
	; 14/03/2021
	dec	eax ; end sector of the partition
...
cpp_21:
	...
	; 14/03/2021
	inc	eax ; end sector address + 1
	mul	dword [hs] ; heads*spt

	;sub	ax, [pp_StartSector]
	;sbb	dx, [pp_StartSector+2]
	; 14/03/2021
	sub	eax, [pp_StartSector]

	;mov	cx, ax
	;mov	bx, dx
	;;jmp	short cpp_16
	; 14/03/2021
	mov	ecx, eax ; partition size
...
cpp_15:
	...
	; 14/03/2021
	; clear -specially- high word of ecx
	xor	ecx, ecx ; (this may not be needed!?)
...
create_extended_partition:
	...
	; 15/03/2021
	mov	eax, [di+free_space.start]

	; 24/10/2020
	;mov	di, ax ; 15/03/2021
...
cep_4:
	...
	; eax = start cylinder
	mov	eax, ebx
	mul	dword [hs] ; heads * sectors
		; eax = start sector, edx = 0
	...
	mov	eax, [ppn_Sectors]
	div	dword [hs] ; heads * spt
		; eax = cylinders, dx = remainder
	and	dx, dx
	jz	short cep_1
	inc	eax ; round up
cep_1:
	; 15/03/2021
	; EBX = extended partition's start cylinder
 	; 16/02/2019
	...
	; 15/03/2021
	mov	edx, eax ; cylinder count
	add	edx, ebx ; result is end cyl + 1
	dec	edx ; end cylinder
	;mov	di, [c_fspc_offset]
...
cep_6:
	...
	mov	eax, [hs] ; [heads] * [sectors]
	mul	edx
		; eax = LBA of end cylinder, head 0, sector 1
		; edx = 0
	;push	dx
	;push	ax
	; 15/03/2021
	push	eax ; (end cylinder)*heads*sectors

	mov	al, ch
	dec	al ; 26/10/2020 ; [heads] - 1
	;mov	cx, [sectors]  ; 63 or 17
	; cl = sectors ; 15/03/2021
	mul	cl
		; ax = (heads-1)*sectors
	; 15/03/2021
	mov	dx, ax ; (heads-1)*sectors
	...
	; 15/03/2021
	xor	ch, ch
	add	dx, cx ; ((heads-1)*sectors)+sectors
	pop	eax ; ((end cylinder)*heads)*sectors
	add	eax, edx
	; eax = (((end cylinder)*heads)+(heads-1))*sectors)+sectors

	;sub	ax, [si+ptStartSector]
	;sbb	dx, [si+ptStartSector+2]
	; 15/03/2021
	sub	eax, [si+ptStartSector]

	;mov	[si+ptSectors], ax
	;mov	[si+ptSectors+2], dx
	; 15/03/2021
	mov	[si+ptSectors], eax
...
; 23/10/2020
	...
	; 15/03/2021
set_partition_id:
	; 07/11/2020 (fdisk4.s, 32 bit registers)
	; 23/10/2020 (fdisk3.s)
	; input:
...
spid_6:
	...
	;jnb	short spid_7
	; 15/03/2021
	cmp	ecx, 400000h ; 2GB (64K*64 sectors)
	jnb	short spid_7
...
spid_13:
	...
	; 15/03/2021
	cmp	ecx, 65536
	jb	short  spid_11 ; force to FAT 16 CHS or FAT 12
	cmp	ecx, 100000h ; 512MB
	jnb	short spid_16  ; OK
	; < 512MB
...
spid_15:
	...
	;cmp	bx, 1
	;jb	short spid_11  ; invalid! (< 32 MB)
	;ja	short spid_8
	;cmp	cx, 1045
	...
	jnb	spid_8   ; OK (>= 33MB)
	cmp	ecx, 65536
	jb	short spid_11  ; invalid! (< 32MB)
;spid_16:
	mov	al, 6
	;retn
	jmp	spid_8 ; 23/10/2020
...
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	; 06/11/2020 (fdisk4)
	; 32 bit division (80386 div instruction for 32 bit regs)
	; will be used instead of div32 procedure for 16 bit regs
;div32:
	;; DX_AX/CX
...
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	; 06/11/2020 (fdisk4)
	; 32 bit multiplication (80386 mul instruction)
	; will be used instead of mul32 procedure for 16 bit regs
;mul32:
	;; DX_AX*CX
	;; Result: BX_DX_AX
	;push	cx
...
B_16:
	...
	movzx	edx, word [sectors]
	sub	eax, edx
	mov	[pp_Sectors], eax  ; = [total_sectors] - [sectors]
B_17:
	cmp	byte [wholedisk], bl ; 0
	ja	B_28 ; 09/02/2019
B_18:
...
;B_60:
	...
	mov	edx, [min_sectors]
	cmp	eax, edx
	jnb	short B_23 ; proper size (for now)
...
B_23:
	...
	; 15/03/2021
	;sub	eax, eax ; no need to clear hw of eax here
...
B_48:
	...
	; 15/03/2021
	; eax = First available space which fits to request
...
C_10:
	...
	; 16/03/2021
	xor	edx, edx ; (this may not be necessary)
	xor	ebx, ebx ; (this may not be necessary)
...
FAT32_f_2:
	...
	; 16/03/2021
	;xor	edx, edx
	mov	dx, [bp+0Eh] ; [BPB_RsvdSecCnt] ; 2 bytes
	sub	eax, edx ; sub eax, 32
		; TmpVal1 = DiskSize - (BPB_ResvdSecCnt +
		;	     		RootDirsectors)
		; RootDirSectors = 0 (for FAT32 FS)
	...
	; 16/03/2021
	mov	[bp+24h], eax	; [BPB_FATSz32]
	; * 2
	shl	eax, 1
	;add	eax, eax
	;EAX = [BPB_NumFATs] * [BPB_FATSz32] ; 2 * FATsz
	mov	cx, [bp+0Eh]	; [BPB_RsvdSecCnt] ; 32
	add	ecx, eax
	;ECX = [BPB_RsvdSecCnt]+[BPB_NumFATs]*[BPB_FATSz32]
	mov	eax, [bp+20h]	; [BPB_TotSec32]
	sub	eax, ecx
	mov	[data_start], ecx
	;EAX = Data sectors
	mov	[data_sectors], eax
	movzx	ecx, byte [bp+0Dh] ; [BPB_SecPerClus]
	sub	edx, edx
	div	ecx
	;EAX = Count of clusters (rounded down)
	mov	[cluster_count], eax
	xor	dl, dl
...
write_format_percent:
	...
	; 16/03/2021
	; EAX = current sector (which has been written)
	...
	; 16/03/2021
	sub	eax, [bp+1Ch]	; [BPB_HiddSec]
wpc_t:
	;mov	cx, [format_percent]
	;call	div32
	; 16/03/2021
	sub	edx, edx
	div	dword [format_percent]
...
FAT16_f_10:
	...
	add	dx, 15 ; DX = 527
	shr	dx, 4 ; /16 = 527/16 = 32
		; ((32*DX)+511)/512
	mov	[root_dir_secs], dx
	add	dx, [bp+0Eh]	; [BPB_RsvdSecCnt] ; 1
	sub	eax, edx
		; TmpVal1 = DiskSize - (BPB_ResvdSecCnt +
		;	     		RootDirsectors)
...
FAT16_f_17:
	...
	; 17/03/2021
	mov	edx, [bp+1Ch]	; [BPB_HiddSec]
	;movzx	ebx, word [bp+13h] ; [BPB_TotSec16]
	mov	bx, [bp+13h] ; [BPB_TotSec16]
...
FAT12_f_3:
	...
	;mov	bx, [data_start+2] ; lba/abs addr
	; 17/03/2021
	mov	edx, [data_start] ; lba/abs addr

	; 11/02/2019
	;sub	cx, [root_dir_secs]
	;sbb	bx, 0
	; 17/03/2021
	;movzx	ebx, word [root_dir_secs]
	mov	bx, [root_dir_secs]
	sub	edx, ebx
	;push	edx ; ***
...
SINGLIX_fs1_f_1:
	...
	; 19/03/2021 (32bit cylinder count <=267349)
	cmp	byte [cylinders+2], 0
	ja	short SINGLIX_fs1_f_2
	mov	cx, [heads]
	mov	ax, [cylinders]
	mul	cx
	and	dx, dx
...
SINGLIX_fs1_f_3:
	...
	; 19/03/2021
	mov	[MAT_VolumeSize], eax ; Total Sectors of the FS
	;mov	[MAT_BeginSector], cx ; Beginning Sector of the FS
	;mov	[MAT_BeginSector+2], bx
	mov	[MAT_BeginSector], edx ; Beginning Sector of the FS
	...
	; 19/03/2021
	; EAX = FS1 Volume Size	
	add	eax, 7 ; 7 bits more (for round up)
	xor	dl, dl ; edx = 0
	...
	; 20/03/2021
	add	eax, 2
	mov	[bp+28], eax  ; [bsRootDirDT] ; RDT address (offset)
	...
	; 20/03/2021
	mov	edx, [MAT_VolumeSize]
	sub	edx, eax
	mov	[MAT_FreeSectors], edx
	mov	[MAT_FirstFreeSector], eax
...
modify_fs_vname_3:
	...
	; 20/03/2021
	xor	dx, dx
	add	eax, [MAT_FreeSectors]
	div	ecx
	mov	[DAT_LFBit], dx
	mov	[DAT_LFSector], eax
...
SINGLIX_fs1_f_13:
	...
	or	al, 1
	or	ah, ah
	jz	short SINGLIX_fs1_f_14
	shl	al, 1
	dec	ah
	jmp	short SINGLIX_fs1_f_13
SINGLIX_fs1_f_14:
	stosb
	;mov	cx, 511
	;sub	cx, bx
...
SINGLIX_fs1_f_9:
	...
	;adc	dx, 0
	; 20/03/2021
	mov	eax, [MAT_BeginSector]
	add	eax, 2 ; [DAT_Address] = 2
	add	eax, esi

	; Write DAT sector(s)
	;; DX_AX = Disk Allocation Table sector address
	; EAX = Disk Allocation Table sector address
...
SINGLIX_fs1_f_10:
	...
	; 20/03/2021
	mov	edx, [bp+16]	; [bsVolumeSize]
	add	edx, [bp+12]	; [bsBeginSector]
...
SINGLIX_fs1_f_11:
	...
	; 20/03/2021
	; (this may not be needed)
	; (but cleaering hw of 32 bit regs may be useful)) 
	xor	edx, edx 
	xor	eax, eax
...
bin_to_decimal:
	...
	; 	till EAX<10.
	;
	; Modified regaisters: ecx (=10), edx (<10)
...
write_number:
	; 21/03/2021
	; 09/03/2021 (fdisk4.s)
	; eax = binary number (must be <=
	; di = decimal number/string location
	; modified registers: eax, ecx, edx, bp, di
	mov	ecx, 10
	xor	edx, edx
	mov	bp, sp
wnum_1:
...
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	; 13/03/2021
	; 12/03/2021 (fdisk4.s, 2TB disk partitioning)
partition_size_input:
	...
	mov	[pSize_multiplier+2], cx ; 0 ; 13/03/2021
	...
	;mov	cl, 100
	mov	cl, 99
	mov	eax, [pp_Sectors]  ; > 0
	add	eax, ecx ; eax = eax + 99
	adc	dl, 0
	inc	cl ; ecx = 100	
	div	ecx
	mov	[pSize_multiplier], eax	; >= 1
	;xor	eax, eax
	;xor	dl, dl

	;mov	ah, 0
	;mov	[msg_psize_unit+1], ah ; 0
	; 13/03/2021
	mov	[msg_psize_unit+1], ch ; 0
...
psu_0:
	...
	; 13/03/2021
	mov	byte [pSize_maxdigits], 6 ; <= 267349 cylinders

	;sub	dh, dh
	mov	[msg_psize_unit+1], dh ; 0
	mov	al, 'C'
	jmp	short psu_6
psu_1:
	cmp	al, 'G'
	...
	;mov	word [pSize_multiplier+2], 1024
	; 13/03/2021
	mov	dword [pSize_multiplier], 1024*1024*2

	;;mov	byte [pSize_maxdigits], 1
	;mov	byte [pSize_maxdigits], 3 ; <= 512 GB ; 23/10/2020
	; 12/03/2021
	mov	byte [pSize_maxdigits], 4 ; <= 2048 GB
	jmp	short psu_6
psu_2:
	cmp	al, 'M'
	jne	short psu_3
...
psu_5:
	...
	; 13/03/2021
	; <= 2147483648 KB or <= 4294967296 sectors
...
psu_7:
	...
	sub	al, '0'
	;xor	ah, ah  ; 13/03/2021 (AH will not be used)
	push	ax	; (*)
	add	al, '0'
...
init_partition_table:
	...
	; 05/11/2020
	mov	cx, 44 ; 22*4 = 88 bytes
	;xor	ax, ax ; 0
	xor	eax, eax ; 09/03/2021
	rep	stosw
...
ipt_1:
	...
	; 09/03/2021
	jmp	short ipt_3 ; non-dos partition (unknown fs type)
ipt_18:
...
;ipt_4_fix:
	...
	mov	al, [si+ptBeginCylinder] ; 09/03/2021
	mov	ah, bh ; 09/03/2021
	;mov	ax, [cylinders]
	;dec	ax
	;cmp	ax, bx
	;jb	short ipt_retn ; invalid
	;mov	[part_table_start_cyl+di], bx
	mov	[part_table_start_cyl+di], eax ; 09/03/2021
	shr	dh, 6
	;mov	dl, [si+ptEndCylinder]
	mov	al, [si+ptEndCylinder] ; 09/03/2021
	mov	ah, dh	; 09/03/2021
	;cmp	ax, dx
	;jb	short ipt_retn ; invalid
	;mov	[part_table_end_cyl+di], dx
	mov	[part_table_end_cyl+di], eax ; 09/03/2021

	;mov	ax, [si+ptStartSector]
	;mov	dx, [si+ptStartSector+2]
	mov	eax, [si+ptStartSector] ; 09/03/2021

	;mov	[part_table_rel_sec_lw+di], ax
	;mov	[part_table_rel_sec_hw+di], dx
	mov	[part_table_rel_sec+di], eax ; 09/03/2021
...
ipt_12:
	...
	mov	eax, [si+ptSectors] ; 09/03/2021
...
ipt_6:
	...
	;jc	short ipt_13
	; 09/03/2021
	add	eax, [part_table_rel_sec+di]
	jc	short ipt_13  ; invalid !
...
ipt_7:
	...
	; 12/03/2021
	mov	eax, 1022
	cmp	[part_table_start_cyl+di], eax
	ja	short ipt_19 ; = 1023
	inc	ax
	cmp	[part_table_end_cyl+di], eax
	jb	short ipt_14  ; < 1023
ipt_19:
	; 27/10/2020
...
; 27/10/2020
	...
	; 09/03/2021
	; 06/11/2020 (32 bit cylinder numbers, upto 267349)
cylindernumberfix:
	; ax = 1022 -> set start cylinder
	; ax = 1023 -> set end cylinder
	...
	; modified registers: eax, edx ; 09/03/2021
	...
	; 09/03/2021
	mov	eax, [part_table_rel_sec+di]
	xor	edx, edx
	div	dword [hs] ; [heads*spt] ; <= 16065

	;cmp	ax, [part_table_start_cyl+di]
	; 09/03/2021
	cmp	eax, [part_table_start_cyl+di]
	;je	short cnf_5
	je	short cnf_1
	jb	short cnf_2 ; invalid
	;mov	[part_table_start_cyl+di], ax ; change it
	; 09/03/2021
	mov	[part_table_start_cyl+di], eax ; change it
	;jmp	short cnf_5 
cnf_1:
...
cnf_5:
	...
	; 09/03/2021
	mov	eax, [part_table_rel_sec+di] ; start sector
	add	eax, [part_table_num_sec+di] ; number of sectors
	;jc	short cnf_2
...
sortpt_6:
	...
	mul	dl ; * 22 ; 05/11/2020

	;xchg	di, ax
	; 10/03/2021
	mov	di, ax
	mov	eax, [part_table_end_cyl+si]

	;cmp	[part_table_end_cyl+di], ax ; previous > current
	; 10/03/2021
	cmp	[part_table_end_cyl+di], eax ; previous > current
...
; 03/02/2019
	...
	; 12/03/2021
	; 11/03/2021 (fdisk4.s) -32 bit cylinder numbers-
	;
find_part_free_space:  ; find/calculate free space for partitions
	...
	; 12/03/2021
	; OUTPUT:
	;   ECX = the largest free space/cylinders (between partitions)
	;    AX = Free space index (gap number) - from 0 to 4 -
	;    BX = Free space structure offset (for the largest free space)
	;   (EDX = 0)
	...
	mov	cx, 5*8 ; (5*16/2)
	;mov	ecx, 5*8 ; 11/03/2021 
	;xor	ax, ax ; 0
	xor	eax, eax ; 11/03/2021
	rep	stosw
...
fpfs_2:
	...
	; 05/11/2020
	mov	al, 22 ; partition table structure size = 22 bytes
	mul	cl
	mov	bx, ax
...
fpfs_4:
	...
	; 11/03/2021
	mov	edx, [part_table_start_cyl+bx]
	cmp	edx, ecx
	jna	fpfs_9
	...
	sub	edx, ecx
	dec	eax
	mov	[free_space.end], eax
	mov	[free_space.space], edx
	...
	; 11/03/2021
	mov	eax, [part_table_rel_sec+bx]
	sub	eax, [free_space.startsector]
	mov	[free_space.sectors_unused], eax
...
fpfs_7:
	...
	; 05/11/2020
	mov	al, 22 ; Partition tables/data structure size = 22 bytes
	mul	cl
	mov	bx, ax  ; ah = 0
	...
	; 12/03/2021
	mov	edx, [part_table_end_cyl+bx]   ; end cyl. of previous partition
	mov	ecx, [part_table_start_cyl+si] ; start cyl. of current partition

	inc	edx ; start cylinder of free space is after the end cylinder of
		    ; the previous partition (as sorted)

	cmp	ecx, edx ; and it must be before than the next partition (as sorted)
	jna	short fpfs_9 ; je short fpfs_9
	...
	mov	[free_space.start+di], edx
	mov	[free_space.end+di], eax
	mov	[free_space.space+di], ecx
	...
	mov	[free_space.sectors_unused+di], edx
...
fpfs_11:
	...
	;mov	cl, 18  ; Partition table structure size = 18 bytes
	; 05/11/2020
	mov	cl, 22  ; Partition table structure size = 22 bytes
	mul	cl
	mov	bx, ax
	;mov	cx, [cylinders]
	;dec	cx ; 15/02/2019 
...
fpfs_12:
	...
	; 12/03/2021
	mov	[free_space.end+di], ecx

	; 05/11/2020
	;mov	ax, [part_table_end_cyl+bx]
	;mov	dx, cx	; cx = end cylinder of the disk
	;sub	dx, ax	; ax = end cylinder of the last partition
	;mov	[free_space.space+di], dx ; di = 4*22 ; 05/11/2020
	;inc	ax
	;mov	[free_space.start+di], ax
	; 12/03/2021
	mov	eax, [part_table_end_cyl+bx]
	; eax = end cylinder of the last partition
	; ecx = end cylinder of the disk
	sub	ecx, eax
	mov	[free_space.space+di], ecx ; di = 4*22
	inc	eax
	mov	[free_space.start+di], eax
	...
	;mov	[free_space.sectors_unused+di], cx
	;mov	[free_space.sectors_unused+2+di], bx
	mov	[free_space.sectors_unused+di], ecx
...
fpfs_14:
	...
	; 12/03/2021
	mov	[free_space.start], edx ; 0 or 1
	...
	;;mov	[free_space.startsector+2], 0
	; 12/03/2021
	mov	[free_space.startsector], eax
	...
	; 12/03/2021
	mov	eax, [cylinders] ; disk capacity
	dec	eax
	mov	[free_space.end], eax
	sub	eax, edx
	inc	eax
	mov	[free_space.space], eax
...
fpfs_16:
	...
	; 12/03/2021
	mov	ecx, edx ; Largest free space
	mov	ax, bx
fpfs_17:
	dec	byte [_i_]
	jz	short fpfs_18
...
fpfs_18:
	...
	; bx = offset (from 0 to 88) ; 12/03/2021
...
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	; 15/03/2021 (fdisk4.s)
	;	((32 bit registers, 32 bit cylinder numbers))
	; 15/02/2019
find_enough_free_sectors:
	...
	; 15/03/2021
	; INPUT:
	;	EAX = Requested space (as non-aligned free sectors)
	;	[freespace_count] = number of free spaces/gaps
	; OUTPUT:
	;	EAX = Available space
	;	If CF = 0 -> EAX >= Request
...
fefss_1:
	...
	; 15/03/2021
	cmp	eax, [bx]
	je	short fefss_8 ; available space
			      ; = requested space
	jb	short fefss_7 ; > requested space
fefss_2:
	; 30/10/2020
	;;cmp	word [bx], 0
	;;jne	short fefss_3
...
fefss_6:
	...
	;mov	dx, di
	; 15/03/2021
	mov	eax, edx ; (max.) available space
			 ; (< requested space)
	sub	edx, edx
	;
	xor	ch, ch
...
;cylinders_to_sectors:
	...
	; 15/03/2021
	; INPUT:
	;	eax = cylinders (Total or partition's cyl count)
	; OUTPUT:
	;	eax = sectors
...
; 03/02/2019
	...
	; 12/03/2021
	; 11/03/2021 (fdisk4.s) - 32 bit cylinder count-
	;
cylinders_to_percent:
	...
	; 11/03/2021
	; INPUT: dword [cylinders] ; number of total disk cylinders
	;	 eax = partition's cylinder count
	; OUTPUT: ax = pencentage
...
ctpc_1:
	...
	cmp	edx, ecx ; is remainder >= total_cylinders/2 ?
	jb	short ctpc_5 ; No.
ctpc_4:
	inc	ax
ctpc_5:
...
display_partition_table_x:
	mov	eax, 5 ; display extended partition table
	jmp	short dpt_x
...
n_pos	equ 30 ; 1 byte	
	...
	; 12/03/2021
	xor	eax, eax ; display primary partition table
dpt_x:
	mov	[_e_], al
...
;fsx_pos equ 61 ; 14 bytes
	...
	;xor	bh, bh
	mov	al, ptbl.size ; 22 ; 05/11/2020
	mul	bl
	add	bp, ax
dpte_0:
...
edit_ext_table_create_x: ; 02/03/2019
	...
	; 13/03/2021
	cmp	dword [lcylinders], 1 ; at least 1 cylinder
	jb	display_extended_pt ; cancel
...
eetc_1:
	...
	;sub	bx, bx ; 0
	; 13/03/2021
	movzx	ecx, word [sectors]

	;mov	ax, [ep_StartSector]
	;mov	dx, [ep_StartSector+2]
	; 13/03/2021
	mov	eax, [ep_StartSector]
	...
	;jb	short eetc_11
	; 13/03/2021
	cmp	dword [lcylinders], 0FFFFFFFFh
	jb	short eetc_11

	;mov	ax, [ep_free_sectors]
	;mov	dx, [ep_free_sectors+2]
	; 13/03/2021
	mov	eax, [ep_free_sectors]
...
eetc_3:
	...
	; 13/03/2021
	sub	eax, edx
	; eax = offset from start of the ep start sector
...
eetc_14:
	...
	;adc	bx, dx
	; 13/03/2021
	add	edx, eax ; ldd's start sector LBA

	;sub	cx, 1
	;sbb	bx, 0
	;; bx:cx = end sector address of ldd
	; 13/03/2021
	dec	edx ; end sector address of ldd
...
;eetc_4:
	...
	; 13/03/2021
	; ecx = cylinder number (0 to 267349)
...
eetc_17:
	...
	; 13/03/2021
	mov	eax, [hs] ; [sectors] * [heads]
	dec	eax ; [sectors] * [heads] - 1
	...
	;	; dx:ax = end cylinder * heads * sectors
	; 13/03/2021
	inc	eax ; heads * sectors
	mul	dword [endcyl]
	; eax = ((cylinders-1) * heads * sectors)
	...
	mov	dl, [sectors]
	dec	dl ; [sectors] - 1
	add	eax, edx  ; + (sectors - 1)

	;mov	cx, [ep_EndSector]
	;mov	bx, [ep_EndSector+2]
	; 13/03/2021
	mov	edx, [ep_EndSector]
...
eetc_6:
	...
	; 05/11/2020
	mov	ah, 22	; primary partition table structure size
	mul	ah
	mov	si, ax
	...
	; 13/03/2021
	mov	eax, [si+part_table_end_cyl]
	mov	[endcyl], eax ; end cylinder of the ep
eetc_19:
	; 03/11/2020
...
eetc_20:
	...
	; 13/03/2021
	sub	eax, [bp+ldd_start]
	sub	eax, [di+ptStartSector]
...
;eetc_18:
	...
	; 05/11/2020
	mov	ah, 22 ; extended partition table structure size
	mul	ah
...
; 05/03/2019
	...
	; 12/03/2021 (fdisk4.s, 32 bit registers)
...
cefs_1:
	; 12/03/2021
	mov	edx, ebx ; edx < 65536
	; eax = free space (sectors)
	retn
...
size_to_fat_type_lba:
	...
	; 13/03/2021
	; eax = DOS Partition Size in sectors
...
gldds_getchar:
	...
	; 12/03/2021
	mov	dword [lcylinders], 0FFFFFFFFh
				 ; sign for all free sectors
	mov	si, _msg_YES
	;call	print_msg
	;retn
	jmp	print_msg
...
gldds_6:
	...
	; 13/03/2021
	; eax = partition size input, edx = 0
	div	dword [hs] ; heads*sectors  ; <= 16065
	; eax = cylinders, dx = remainder
	or	dx, dx
	jz	short gldds_7
	inc	eax ; +1
...
;LDD 5 <- LDD_START3, ebr 2nd entry (LDD 5 is not displayed)
	...
	; 05/11/2020
	mov	ah, 22 ; partition table structure size
	mul	ah
	;mov	si, part_table_rel_sec_lw
	mov	si, part_table_rel_sec ; 09/03/2021
	add	si, ax
	...
	; 09/03/2021
	mov	eax, [si]
	mov	[ep_StartSector], eax
	mov	edx, [si+4]
	mov	[ep_Size], edx
	add	edx, eax
	dec	edx
	mov	[ep_EndSector], edx
	xor	edx, edx
	...
	; 09/03/2021
	; eax = Extended partition address 
	; es:bx = Extended partition buffer
...
TrDOS_Welcome:
	...
	db	"FDISK4 v2.0.260129 (c) Erdogan TAN 2021-2026"
	db	0Dh,0Ah
	db	0Dh,0Ah
	db	0
TrDOS_Usage:
	db	'Usage: fdisk4r5 <hard disk name> '
	db	0Dh, 0Ah
	db	0Dh, 0Ah
	db	"Hard disk names: "
	db	0Dh, 0Ah
...
; 4 partitions
; 22 byte partition data structure per partition
; 88 bytes (4*22)
