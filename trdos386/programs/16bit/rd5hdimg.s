; ****************************************************************************
; RD5HDIMG.S (RD5HDIMG.COM) - Retro DOS v5 Hard Disk Image Formatting Utility
; 							  (for MSDOS/WINDOWS)
; ****************************************************************************
...
; ----------------------------------------------------------------------------
; Modified from 'hdimage.s'(HDIMAGE.COM) source code by Erdogan Tan
; (03/05/2024) - TRDOS 386 v2 hard disk image formatting utility -
; ****************************************************************************
; nasm hdimage.s -l hdimage.txt -o HDIMAGE.COM
...
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	; 04/05/2024
	mov	si, retrodos5_disk_chs_header
	;call	print_msg
	;
	;jmp	A_27
	; 20/09/2020
...
cpp_5:
	...
	mov	[si+ptStartSector], ax
	;mov	[si+ptStartSector+2], cx ; 0

	mov	[pp_StartSector], ax
	;mov	[pp_StartSector+2], cx
...
B_66:
	...
	mov	si, msg_create_dos_partition_h ; header
	call	print_msg
	mov	si, msg_create_dos_partition_s ; size options
	call	print_msg
B_67:
	xor	ah, ah
	int	16h
...
;FAT32_lba_format:
	...
	mov	bp, RD5_FAT32_hd_bs ; 04/05/2024
	lea	di, [bp+3]
	mov	si, bs_oem_name
	mov	cx, 4
	rep	movsw 
...
FAT32_f_3:
	...
	mov	bx, RD5_FAT32_hd_bs ; 04/05/2024
	...
	mov	bx, RD5_FAT32_hd_bs + 512 ; 04/05/2024
...
;FAT16_lba_format:
	...
	mov	bp, RD5_FAT16_hd_bs ; 04/05/2024
	lea	di, [bp+3]
	mov	si, bs_oem_name
	mov	cx, 4
	rep	movsw 
...
FAT16_f_11:
	...
	mov	bx, RD5_FAT16_hd_bs ; 04/05/2024
...
FAT12_hdi_format:
	...
	mov	bp, RD5_FAT12_hd_bs ; 04/05/2024
	lea	di, [bp+3]
	mov	si, bs_oem_name
	mov	cx, 4
	rep	movsw
...
FAT12_f_9:
	...
	mov	bx, RD5_FAT12_hd_bs ; 04/05/2024
	; ES:BX = Boot Sector Buffer
	call	write_hd_sector
	jc	formatting_error
	call	write_format_percent
...
RD5_Welcome:
	...
	db	'Retro DOS v5 Fixed Disk Image Format Utility'
	db	0Dh, 0Ah
	;;db	"v1.1.240503 (c) Erdogan TAN 2019-2024"
	;db	"v2.0.240715 (c) Erdogan TAN 2019-2024"
	db	"v2.0.260129 (c) Erdogan TAN 2019-2026"
	db	0Dh,0Ah
	db	0Dh,0Ah
	;db	'Usage: hdimage <image file name> '
	db	'Usage: rd5hdimg <image file name> '
	db	0
...
retrodos5_disk_chs_header:
	db	0Dh, 0Ah
	;db	"                     TRDOS 386 (SINGLIX) HARD DISK IMAGE                        "
	; 04/05/2024
	db	"                         RETRO DOS V5 HARD DISK IMAGE                           "
	db 	0
...
msg_c_ldd_unused_warning:
	db	0Dh, 0Ah
	db	"There is unused extended partition space after logical dos drive 4 !"
	db 	0Dh, 0Ah
