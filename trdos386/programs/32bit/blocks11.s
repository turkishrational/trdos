; ****************************************************************************
; blocks11.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 28/02/2021
;
; ****************************************************************************
; nasm blocks11.s -l blocks11.txt -o BLOCKS11.PRG -Z error.txt
; (modified from 'blocks10.s', 28/02/2021)

; 'sysvideo' bh = 2, block copy and modification test (VESA VBE mode 112h)
; (640*480, 16 bit high colors version)
...
set_vesa_mode_112h_ok:
	...
	mov	ecx, 120*160
	mov	eax, 0FFFFh ; white 
	mov	edi, whiteblock
	rep	stosw

	mov	cx, 120*160
	mov	ax, 001Fh ; blue ; 00000000 00011111b 
	mov	edi, blueblock
	rep	stosw

	mov	cx, 120*160
	mov	ax, 0F800h ; red ; 11111000 00000000b 
	mov	edi, redblock
	rep	stosw

	mov	cx, 120*160
	mov	ax, 07E0h ; green ; 00000111 11100000b 
	mov	edi, greenblock
	rep	stosw

	mov	cx, 120*160
	mov	ax, 0FFE0h ; yellow ; 11111111 11100000b
	mov	edi, yellowblock
	rep	stosw
...
blackloop:
	...
	; Green
	mov	ecx, 640*110
	mov	ax, 11111100000b ; Green
	rep	stosw

	; Yellow
	mov	ecx, 640*110
	mov	ax, 1111111111100000b ; Yellow
	rep	stosw
