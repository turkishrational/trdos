; ****************************************************************************
; circle14.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 18/02/2021
;
; ****************************************************************************
; nasm circle14.s -l circle14.txt -o CIRCLE14.PRG -Z error.txt
; (modified from 'circle13.s', 18/02/2021)
...
START_CODE:
	...
	; convert 8x8 fonts to 16x16 fonts
...
p_o_1:
	mov	ebp, txt_x0
	mov	esi, 001C0008h ; row 28, column 8
	call	p_d_x
	mov	eax, [prevx] ; [_x0]
	mov	edi, val_x0
	call	num_to_txt
...
p_o_2:
	mov	ebp, txt_y0
	mov	esi, 00300008h ; row 48, column 8
	call	p_d_x
	mov	eax, [prevy] ; [_y0]
	mov	edi, val_y0
	call	num_to_txt
...
p_d_x_n:
	...
	and	dl, dl
	jz	short p_d_x_ok
	shl	edx, 5 ; * 32 (for 16x16 user font)
	...
	;sys	_video, 018Fh, [tcolor], 8002h
			;; use STD VGA video memory
			;; (0A0000h)
	sys	_video, 020Fh, [tcolor], 8002h ; 16x16 user font
		 ; use LFB for current VBE mode
		 ; for writing fonts on screen	
	inc	ebp
	add	si, 17 ; next char pos
	jmp	short p_d_x_n
p_d_x_ok:
	retn
...
bss_start:
fontbuff2:
	resb 2*256*16 ; 16x16 font data (modif. from 8x8)
fontbuff1:
