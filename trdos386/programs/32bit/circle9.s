; ****************************************************************************
; circle9.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 16/02/2021
;
; ****************************************************************************
; nasm circle9.s -l circle9.txt -o CIRCLE9.PRG -Z error.txt
; (modified from 'circle8.s', 16/02/2021)


; Draw circle by using 'sysvideo' bx=0305h
; and show diameter and origin by using bx=020Fh
...
p_o_1:
	mov	ebp, txt_x0
	mov	esi, 00100004h ; row 16, column 4
	call	p_d_x
	mov	eax, [prevx] ; [_x0]
	mov	edi, val_x0
	call	num_to_txt
...
p_o_2:
	mov	ebp, txt_y0
	mov	esi, 001A0004h ; row 26, column 4
	call	p_d_x
	mov	eax, [prevy] ; [_y0]
	mov	edi, val_y0
	call	num_to_txt
...
p_d_1:
	...
	mov	esi, 00040004h ; row 4, column 4
	call	p_d_x
...
p_d_x_n:
	...
	;sys	_video, 018Fh, [tcolor] ; use STD VGA video memory
					; (0A0000h)
