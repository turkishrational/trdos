; ****************************************************************************
; blocks2.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 22/02/2021
;
; ****************************************************************************
; nasm blocks2.s -l blocks2.txt -o BLOCKS2.PRG -Z error.txt
; (modified from 'blocks1.s', 21/02/2021)
...
set_vesa_mode_101h_ok:
	...
	; apply NOT to pixel colors of
	; the window/block on row 70, column 120 with 
	; block size 120(w)*160(h). 
	mov	ecx, (70*65536)+120
	mov	edx, (160*65536)+120 
	mov	ebx, 0217h ; non masked window NOT op
	sys	_video

	call	waitforkey

	; NOT to window's pixel colors again
	mov	ecx, (70*65536)+120
	;mov	edx, (160*65536)+120 
	;mov	ebx, 0217h ; non masked window NOT op
	sys	_video
	...
	; apply NOT to pixel colors of
	; the window/block on row 160, column 260
	mov	ecx, (160*65536)+260
	;mov	edx, (160*65536)+120 
	;mov	ebx, 0217h ; non masked window NOT op
	sys	_video
	...
	; apply NOT to pixel colors of
	; the window/block on row 250, column 120 
	mov	ecx, (250*65536)+120
	;mov	edx, (160*65536)+120 
	;mov	ebx, 0210h ; non masked window copy
	sys	_video
	...
	; apply NOT to pixel colors of
	; the window/block on row 70, column 400
	mov	ecx, (70*65536)+400
	;mov	edx, (160*65536)+120
	;mov	ebx, 0217h ; non masked window NOT op
	sys	_video
	...
	; apply NOT to pixel colors of
	; the window/block on row 250, column 400
	mov	ecx, (250*65536)+400
	;mov	edx, (160*65536)+120
	;mov	ebx, 0217h ; non masked window NOT op
	sys	_video
	...
	; apply NEGate to pixel colors of
	; the window/block on row 70, column 120 with 
	; block size 120(w)*160(h). 
	mov	ecx, (70*65536)+120
	;mov	edx, (160*65536)+120 
	...
	; NEGate window's pixel colors again
	mov	ecx, (70*65536)+120
	;mov	edx, (160*65536)+120 
	;mov	ebx, 0217h ; non masked window NOT op
	sys	_video
	...
	mov	ecx, (250*65536)+120
	;mov	edx, (160*65536)+120 
	;mov	ebx, 0218h ; non masked window NEG op
	sys	_video
...
_0:
	mov	edi, 8
_0_n:
	; INCrease pixel colors of
	; the window/block on row 160, column 260
	mov	ecx, (160*65536)+260
	;mov	edx, (160*65536)+120 
	mov	ebx, 0219h ; non masked window INC op
	sys	_video
...
_1:
	mov	edi, 8
_1_n:
	; DECrease pixel colors of
	; the window/block on row 160, column 260
	mov	ecx, (160*65536)+260
	;mov	edx, (160*65536)+120 
	mov	ebx, 021Ah ; non masked window DEC op
	sys	_video
