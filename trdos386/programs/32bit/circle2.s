; ****************************************************************************
; circle2.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 14/02/2021
;
; ****************************************************************************
; nasm circle2.s -l circle2.txt -o CIRCLE2.PRG -Z error.txt
; (modified from 'circle1.s', 14/02/2021)

; Draw circle by using 'sysvideo' bh=3 (VESA VBE mode version)
...
reset_diameter:
	mov	eax, 240 ; initial diameter 
newdiameter:	
	; Set radius to 240
	;mov	dword [radius], 240
	;mov	dword [_r2], 57600
	mov	[radius], eax
	mov	ebx, eax
	mul	ebx
