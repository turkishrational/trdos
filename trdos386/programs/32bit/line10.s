; ****************************************************************************
; line10.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 20/02/2021
;
; ****************************************************************************
; nasm line10.s -l line10.txt -o LINE10.PRG -Z error.txt
; (modified from 'line9.s', 20/02/2021)

; Draw line by using 'sysvideo' bx=0305h (VESA VBE mode 101h)
...
blackline:
	push	eax
	;xor	al, al
	mov	al, [counter] ; color effect for 'line10.s'
	xchg	[color], al ; color = 0	
	mov	[pcolor], al
	inc	byte [counter] ; color effect for 'line10.s'
	call	writeline
	mov	al, [pcolor]
...
pixelcount:
	resd 1
_fx:	resd 250 ; For every X values from 0 to 250
	resd 1000-250 ; additional buffer
bss_end: