; ****************************************************************************
; loadbmp1.s (TRDOS 386, TRDOS v2.0 - sample binary file, 'loadbmp1.prg')
; ----------------------------------------------------------------------------
...
;
; Assembler: NASM 2.15 
;   command: nasm loadbmp1.s -l loadbmp1.txt -o LOADBMP1.PRG -Z error.txt
;
; Note: This program displays 640x480 8bpp non-compressed BMP files only.
; ****************************************************************************
...
START_CODE:
	...
	; DIRECT LINEAR FRAME BUFFER ACCESS
 	...
 	;inc	bl ; bl = 01h -> 101h, VESA MODE 101h
	mov	bx, 601h  ; Direct access/map to LFB for VBE video mode 101h
	;mov    eax, _video ; 1Fh
...
set_vesa_vbe_mode:
	...
	mov	bx, 4101h ; vbe mode 101h with LFB 
...
; The linear address of the pixel is y*640+x
	mov	eax, 640
	mul	dword [Y] ; calculating linear = y*640
	; 19/12/2020
	mov	ebx, [LFB_addr]
	add	ebx, eax
...
msg_program:
	db 0Dh, 0Ah
	db "LOADBMP1.PRG /// TRDOS 386 VESA VBE function test program"
	db 0Dh, 0Ah
	db "by Erdogan Tan, 20/12/2020", 0Dh, 0Ah
	db 0Dh, 0Ah, 0Dh, 0Ah
