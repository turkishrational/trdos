; ****************************************************************************
; aclock5.s - TRDOS 386 (TRDOS v2.0) Kernel - Analog Clock Demo - Mode 105h
; ----------------------------------------------------------------------------
;
...
;
; ****************************************************************************
; Modified from 'aclock4.s'. (Direct Access to Linear Frame Buffer)

; (Analog Clock display code without FPU instructions) -VESA VBE Mode 105h-
; ((Only INT 40h system calls))
; Drawing lines via SYSVIDEO system call (without using direct LFB access).
...
; ------------------------------------------------------------
; VESA VBE Mode 105h - 1024*768 256 pixels color

draw_centermark:
	mov	esi, centermark
;	mov	ecx, centermarksize
;	shr	ecx, 2
;	mov	edi, [LFB_Address]
;dcm_@:
;	lodsd
;	mov	byte [edi+eax], 0Fh  ; white
;	loop	dcm_@
;	retn

	mov	edx, centermarksize
	shr	edx, 2

	; edx = pixel count
	; esi = user's single color pixel buffer address
	sys	_video, 0305h, 0Fh
	retn

centermark:
	     dd (1024*382)+510,(1024*382)+511
dd (1024*383)+509,(1024*383)+510,(1024*383)+511,(1024*383)+512
dd (1024*384)+509,(1024*384)+510,(1024*384)+511,(1024*384)+512
	     dd (1024*385)+510,(1024*385)+511

centermarksize equ $-centermark
