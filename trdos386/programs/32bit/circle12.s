; ****************************************************************************
; circle12.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 18/02/2021
;
; ****************************************************************************
; nasm circle12.s -l circle12.txt -o CIRCLE12.PRG -Z error.txt
; (modified from 'circle10.s', 18/02/2021)
...
START_CODE:
	...
	; convert 8x14 fonts to 24x48 fonts
	; by inserting 2 empty rows to each characters
	; and then scaling each font
	;mov	esi, fontbuff1
	mov	edi, fontbuff2
	;mov	cx, 256
fontconvert:
...
p_o_2:
	mov	ebp, txt_y0
	mov	esi, 00680008h ; row 104, column 8
	call	p_d_x
	mov	eax, [prevy] ; [_y0]
	mov	edi, val_y0
	call	num_to_txt
...
bss_start:
fontbuff2:
	resb 3*256*48 ; 24x48 font data (modif. from 8x14)
fontbuff1:
