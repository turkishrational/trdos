; ****************************************************************************
; line14.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 07/03/2021
;
; ****************************************************************************
; nasm line14.s -l line14.txt -o LINE14.PRG -Z error.txt
; (modified from 'line13.s', 11/02/2021)

; Draw lines by using 'sysvideo' bx=020Eh (VESA VBE mode version)
; (640*480, 24 bpp version)
...
_8:
	...
	jne	short _9
	call	clear_lines
	mov	dword [color], 0FF7F00h ; orange
	mov	cl, 64
	mov	[length], cl
	call	movecenter
	call	drawline_x
...
_12:
	cmp	ah, 49h  ; Page UP
	jne	short _13
	sub	dword [color], 102030h
	jmp	short _14
_13:
	cmp	ah, 51h  ; Page Down
	jne	waitforkey
	add	dword [color], 102030h
_14:
	call	drawline_x
	jmp	waitforkey
