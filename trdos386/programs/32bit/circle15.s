; ****************************************************************************
; circle15.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 18/02/2021
;
; ****************************************************************************
; nasm circle15.s -l circle15.txt -o CIRCLE15.PRG -Z error.txt
; (modified from 'circle14.s', 18/02/2021)
...
START_CODE:
	...
	; Read (copy) 8x8 system fonts
	mov	esi, fontbuff1
	sys	_video, 0C02h, 256, 0

	; convert 8x8 fonts to 32x32 fonts
...
p_o_1:
	mov	ebp, txt_x0
	mov	esi, 002C0008h ; row 44, column 8
	call	p_d_x
	mov	eax, [prevx] ; [_x0]
	mov	edi, val_x0
	call	num_to_txt
...
p_o_2:
	mov	ebp, txt_y0
	mov	esi, 00500008h ; row 80, column 8
	call	p_d_x
	mov	eax, [prevy] ; [_y0]
	mov	edi, val_y0
	call	num_to_txt
...
p_d_x_n:
	...
	and	dl, dl
	jz	short p_d_x_ok
	shl	edx, 7 ; * 128 (for 32x32 user font)
	...
	;sys	_video, 018Fh, [tcolor], 8005h
			...
			;; (0A0000h)
	sys	_video, 020Fh, [tcolor], 8006h ; 32x32 user font
		 ; use LFB for current VBE mode
		 ; for writing fonts on screen	
	inc	ebp
	add	si, 34 ; next char pos
	jmp	short p_d_x_n
p_d_x_ok:
	retn
...
bss_start:
fontbuff2:
	resb 4*256*32 ; 32x32 font data (modif. from 8x8)
fontbuff1:
	resb 256*8 ; 8x8 font data (from system)
_squares:
