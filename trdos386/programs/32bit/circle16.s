; ****************************************************************************
; circle16.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 19/02/2021 (18/02/2021)
;
; ****************************************************************************
; nasm circle16.s -l circle16.txt -o CIRCLE16.PRG -Z error.txt
; (modified from 'circle15.s', 18/02/2021)
...
START_CODE:
	...
	; Read (copy) 8x16 system fonts
	mov	esi, fontbuff1
	sys	_video, 0C04h, 256, 0

	; convert 8x16 fonts to 32x64 fonts
	; by scaling each font
	;mov	esi, fontbuff1
	mov	edi, fontbuff2
	;mov	cx, 256
fontconvert:
...
set_vesa_mode_105h_ok:
	; Set squares of numbers from 0 to 255
	mov	edi, _squares
	mov	ecx, 255
	mov	ebx, 1
_ss_x:
	...
	;mov	byte [prevd], 510
	;mov	byte [prevx], 1024/2 
	;mov	byte [prevy], 768/2
...
p_o_1:
	mov	ebp, txt_x0
	mov	esi, 004C0008h ; row 76, column 8
	call	p_d_x
	mov	eax, [prevx] ; [_x0]
	mov	edi, val_x0
	call	num_to_txt
...
p_o_2:
	mov	ebp, txt_y0
	mov	esi, 00900008h ; row 144, column 8
	call	p_d_x
	mov	eax, [prevy] ; [_y0]
	mov	edi, val_y0
	call	num_to_txt
...
p_d_x_n:
	...
	and	dh, dh
	jz	short p_d_x_ok
	; edx = 256 * byte [ebp] ; 32x64 font offset
	mov	edi, fontbuff2 ; start of user font data
	add	edi, edx ; add offset for 32x64 user font	
	...
	;sys	_video, 018Fh, [tcolor], 8005h
			;; use STD VGA video memory
			;; (0A0000h)
	sys	_video, 020Fh, [tcolor], 8007h ; 32x64 user font
...
circle_parameters:
	;db "Diameter: 510 pixels", 0Dh, 0Ah
	;db "x0: 512", 0Dh, 0Ah
	;db "y0: 384", 0Dh, 0Ah, 0
txt_diameter:
	db "Diameter: ", 0
...
bss_start:
fontbuff2:
	resb 4*256*64 ; 32x64 font data (modif. from 8x16)
fontbuff1:
	resb 256*16 ; 8x16 font data (from system)
_squares:
