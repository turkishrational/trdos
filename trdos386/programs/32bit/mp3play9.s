; ****************************************************************************
; MP3PLAY9.PRG - MP3 DECODER/PLAYER 1.0 for TRDOS 386 Operating System
; ----------------------------------------------------------------------------
; Based on 
...
;
; Modified from 'mp3play6.s' (14/02/2025) and 'mp3play8.s' (28/02/2025)
;
; ----------------------------------------------------------------------------
...
; ===========================================================================
		...
		; 28/02/2025 (mp3play9.s)
		; 25/02/2025 (mp3play8.s)
...
.err:
		;mov	edx, txt_audio_nf_err
                ;call    wrstr_edx
		; 22/02/2025
		sys	_msg, txt_audio_nf_err, 255, 0Ch
...
; =============== S U B R O U T I N E =======================================
		...
		; 23/02/2025
		; 22/02/2025 (mp3play7.s)
		; 29/01/2025 (mp3play5.s - mp3lay6.s)
		; 28/01/2025
		; 27/01/2025
		; 13/01/2025 (mp3play4.s)
		; 12/01/2025
audio_system_init:
		; 28/01/2025
		mov	eax, [mp3_samples_output_size]
...
init_error:
		call	set_text_mode
		sys	_msg, txt_audio_init_err, 255, 0Ch ; red
                stc
		retn
...
; =============== S U B R O U T I N E =======================================
	...
	; 28/02/2025 (mp3play9.s)
	; 13/02/2025 (mp3play6.s) - 320*200, 256 colors
...
move_forward:
		...
		sub	edx, [fb_count]
		cmp	eax, edx
		jna	short move_bw_or_fw
move_f@:
		mov	eax, edx ; offset
		jmp	short move_bw_or_fw
...
move_bw_or_fw:
		...
		cmp	byte [srb], 0
		jna	short mfb_sp@ ; proper to change
		cmp	bl, 0
		jna	short mfb_sp@@@
...
clear_keyb_buf:
		; clear keyboard buffer
		;mov	ah, 1
		mov	ah, 11h ; 25/02/2025
		int	32h
		jz	short mfb_sp_ok
		;mov	ah, 0
		mov	ah, 10h ; 25/02/2025
		int	32h
		jmp	short clear_keyb_buf
...
txt_hello       db 13,10
		...
		; 28/02/2025
		db 'NOCASH MP3 PLAYER v1.0c+ for TRDOS386 ',0D,0Ah,0
txt_bdate	db 'February 2025.',13,10,0

		; 26/02/2025
txt_help	db 0Dh, 0Ah
		db 'usage: MP3PLAY <FileName1> <FileName2> <...> ',0Dh,0Ah,0
...
; 13/02/2025
; 23/11/2024
colors:		db 0Bh, 0Ah, 0Ch, 0Eh, 09h, 0Dh, 0Fh
			; cyan, green, red, yellow, blue, magenta, white
ccolor:		db 0Bh	; cyan
...
argv		rd 1	; current argument (mp3 file) ptr
argvf		rd 1	; 1st argument (mp3 file) ptr
argvl		rd 1	; last argument (mp3 file) ptr
;
playback_f	rd 1	; playback frequency
playback_c	rd 1	; playback num of channels (byte)
...
; 12/02/2025 (mp3pkay6.s)
prev_points	rd 320 ; previous wave points (which are lighting)

; 28/02/2025 (mp3play9.s)
; 25/02/2025 (mp3play8.s)
; 22/02/2025 (mp3play7.s)
mp3_file_name	rb 80 ; wave file, path name (<= 80 bytes)
		rd 1
