; ****************************************************************************
; circle13.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 18/02/2021
;
; ****************************************************************************
; nasm circle13.s -l circle13.txt -o CIRCLE13.PRG -Z error.txt
; (modified from 'circle12.s', 18/02/2021)
...
START_CODE:
	...
	; convert 8x16 fonts to 24x48 fonts
...
p_o_1:
	mov	ebp, txt_x0
	mov	esi, 00400008h ; row 64, column 8
	call	p_d_x
	mov	eax, [prevx] ; [_x0]
	mov	edi, val_x0
	call	num_to_txt
...
p_o_2:
	mov	ebp, txt_y0
	mov	esi, 00780008h ; row 120, column 8
	call	p_d_x
	mov	eax, [prevy] ; [_y0]
	mov	edi, val_y0
	call	num_to_txt
...
p_d_x_n:
	...
	sub	eax, eax
	mov	al, 48*3
	mul	dl ; * 144 (for 24x48 user font)
	...
	;sys	_video, 018Fh, [tcolor], 8005h
			...
			;; (0A0000h)
	sys	_video, 020Fh, [tcolor], 8005h ; 24x48 user font
		 ; use LFB for current VBE mode
		 ; for writing fonts on screen	
	inc	ebp
	add	si, 27 ; next char pos
	jmp	short p_d_x_n
p_d_x_ok:
	retn
...
bss_start:
fontbuff2:
	resb 3*256*48 ; 24x48 font data (modif. from 8x16)
fontbuff1:
