; ****************************************************************************
; blocks14.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 01/03/2021
;
; ****************************************************************************
; nasm blocks14.s -l blocks14.txt -o BLOCKS14.PRG -Z error.txt
; (modified from 'blocks13.s', 01/03/2021 & 'blocks11.s', 28/02/2021)
...
set_vesa_mode_111h_ok:
	...
	; full screen replace color (replace white colors)
	mov	ecx, 0FFFFh ; WHITE
	mov	edx, 1111111111100000b ; YELLOW
	mov	bl, 0Ch
	sys	_video
	...
	; full screen replace color (replace white colors)
	mov	cx, 0FFFFh ; WHITE
	mov	dx, 1111111111100000b ; YELLOW
	mov	bl, 0Ch
	sys	_video
	...
	; full screen replace color (replace yellow colors)
	mov	cx, 1111111111100000b ; YELLOW
	mov	dx, 1111100000000000b ; RED
	;mov	bl, 0Ch
	sys	_video
	...
	mov	edi, fullscreen_buffer + ((20*640)+40)*2
	...
	mov	ecx, 0FFFFh ; WHITE (current color)
	...
	mov	edi, fullscreen_buffer + ((130*640)+40)*2
	...
	mov	edi, fullscreen_buffer + (350*640)*2
	...
	mov	edi, fullscreen_buffer + (240*640)*2
	...
	; replace color
	mov	cx, 1111111111100000b 
			; YELLOW (current color)
	mov	dx, 11111b ; BLUE (new color)
	...
	mov	esi, fullscreen_buffer+((20*640)+40)*2
	...
	mov	edi, fullscreen_buffer+(20*640)*2
	...
	mov	esi, fullscreen_buffer+((130*640)+40)*2
	;mov	ebx, 0210h ; copy from user to sys 
	mov	bl, 10h
	sys	_video
	...
	mov	ecx, 130*65536
	mov	edx, 110*65536+640
	mov	edi, fullscreen_buffer+(130*640)*2
	;mov	ebx, 0241h ; copy from sys to user 
	mov	bl, 41h
	sys	_video
