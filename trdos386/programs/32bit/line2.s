; ****************************************************************************
; line2.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel w/r tests
; ----------------------------------------------------------------------------
;
; 13/02/2021 (10/02/2021)
;
; ****************************************************************************
; nasm line2.s -l line2.txt -o LINE2.PRG -Z error.txt
; (modified from 'line1.s', 09/02/2021)

; Draw lines by using 'sysvideo' bh=3 (VESA VBE mode version)
...
_draw_hline1:
	...
	; ebp = length
_dhline1_np:
	sys	_video, 0301h, [color], edi
	dec	ebp
	jz	short _dhline1_ok
	inc	edi
	jmp	short _dhline1_np
_dhline1_ok:
	retn
_dvline1:
	cmp	byte [lphase], 1
...
_dvline1_np:
	sys	_video, 0301h, [color], edi
	dec	ebp
	jz	short _dvline1_ok
	add	edi, 640
	jmp	short _dvline1_np
_dvline1_ok:
	retn
_dvline2:
	cmp	byte [lphase], 2
...
_dhline2_np:
	sys	_video, 0301h, [color], edi
	dec	ebp
	jz	short _dhline2_ok
	dec	edi
	jmp	short _dhline2_np
_dhline2_ok:
	retn
_dvline3:
...
_dvline2_np:
	sys	_video, 0301h, [color], edi
	dec	ebp
	jz	short _dvline2_ok
	sub	edi, 640
	jmp	short _dvline2_np
_dvline2_ok:
	retn
