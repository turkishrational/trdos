; ****************************************************************************
; hello.s (TRDOS 386, TRDOS v2.0 - sample binary file, 'hello.prg')
; ----------------------------------------------------------------------------
; HELLO.PRG ! TEST program !
;
; 29/04/2016
;
; Derived from 'hello.s' source code for Retro UNIX 386 v1
;
...
START_CODE:
	...
	;
	sys 	_ver ; get TRDOS version
	add	ax, '00'
	mov	[ver_str_major], ah
	mov	[ver_str_minor], al
	
	sys	_msg, msg_Hello, msg_size, msg_color
	sys 	_exit
here:
	jmp	short here
...
msg_Hello:	db 07h ; beep
		...
		db 'Hello world !'
		db 0Dh, 0Ah
		db 'This is the first program for TRDOS 386 v'
ver_str_major:	db '0'
		db '.'
ver_str_minor:  db '0'
		db ' !'
