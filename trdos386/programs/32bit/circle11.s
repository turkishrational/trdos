; ****************************************************************************
; circle11.s - TRDOS 386 (TRDOS v2.0.3) Test Program - 'sysvideo' pixel tests
; ----------------------------------------------------------------------------
;
; 18/02/2021
;
; ****************************************************************************
; nasm circle11.s -l circle11.txt -o CIRCLE11.PRG -Z error.txt
; (modified from 'circle7.s', 16/02/2021)

; Draw circle by using 'sysvideo' bx=0305h 
; and show diameter and origin by using bx=020Fh (VESA VBE mode version)
; (from 8*16 to 32*64 font conversion, scaled font version of 'circle7.s')
...
_ss_x:
	...
	;mov	word [prevd], 510
	;mov	word [prevx], 1024/2 
	;mov	word [prevy], 768/2
...
p_o_1:
	mov	ebp, txt_x0
	;mov	esi, 00200008h ; row 32, column 8
...
p_o_2:
	mov	ebp, txt_y0
	;mov	esi, 00380008h ; row 56, column 8
	mov	esi, 00980008h ; row 152, column 8
	call	p_d_x
	mov	eax, [prevy] ; [_y0]
	mov	edi, val_y0
	call	num_to_txt
