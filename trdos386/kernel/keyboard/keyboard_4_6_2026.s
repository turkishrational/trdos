...
; ----------------------------------------------------------------------------
; Last Update: 04/06/2026 (Previous: 07/08/2022, v2.0.5)
; ----------------------------------------------------------------------------
...
cpu_reset:
	; 04/06/2026
	; Modern PCI/ACPI motherboard pulse sequence combined with legacy 8042 pulse
...
.wait_8042:
	...
	test	al, 02h			; Wait for Input Buffer Empty
	...
	; 04/06/2026
	; Fallback to Port 92h (Fast Reset System for 32-bit AT clones)
...
k31a:
	; 04/06/2026
	; CRITICAL TIMING FIX: Do not purge E0 early here, pass through cleanly
	;and	byte [KB_FLAG_3], ~LC_E0 ; Purge Extended Key tracking bit
...
k35c:
	; 04/06/2026
	;and	byte [KB_FLAG_3], ~LC_E0 ; Reset Extended State flag prior to push
...
_K60:
	; 04/06/2026
	; --- CRITICAL FIX: ISOLATE EXTENDED KEYS FROM VIRTUAL CONSOLE HOTKEYS ---

	test	byte [KB_FLAG_3], LC_E0	; Is this an extended key (like directional arrows)?

	jnz	short K61		; YES: Skip Alt+F1-F8 console page switches entirely!
...
kb_buffer_full_silent:
	; 04/06/2026
	; --- SAFETY PURGE ON OVERFLOW ---
	and	byte [KB_FLAG_3], ~(LC_E0+LC_E1)
					; Clear tracking states to prevent stickiness
	jmp	near K26
...
; ============================================================================
align 4
	; 04/06/2026 (corrected by Google AI)
; ----------------------------------------------------------------------------

; CRITICAL FIX FOR REAL motherboards: DIRECT POLLED TRANSMISSION FOR INTERRUPT CONTEXT
; ----------------------------------------------------------------------------	
SND_DATA: ; It sends a command/data byte to the keyboard and waits for an ACK.
	...
	push	ebx
	push	ecx
	mov	bh, al			; Command/Data byte to send
...
;SD1:
	...
	; 04/06/2026
	; 3. Wait for a direct ACK (0FAh) response from the output buffer.
	mov	ecx, 80000h
...
SD1:	; .wait_obf:
	...
	test	al, OUTP_BUF_FULL       ; Bit 0: Did data arrive?
	jz	short SD4 ; .no_data
	...
	je	short SD3 ; .success	; Yes, completed successfully
	cmp	al, 0FEh		; Resend ?

	je	short SD2 ; .retry_dec	; Try again
SD4: 	; .no_data:
...
SD2:	; .retry_dec
	...
	jnz	short SD0 ; .retry_loop ; Loop back for retransmission
...
SD3:	; .success:
	...
	and	byte [KB_FLAG_2], ~KB_ERR ; Hatayý temizle
...
align 4	
	; 04/06/2026
SND_LED: ; Updates keyboard LED status using Synchronous Safe Mode
	;---------------------------------------------------------------------
	; SND_LED
	...
	out	20h, al 		; Send EOI to Master PIC immediately!

	;jmp	short SL0		; Go send mode indicator command safely
...
SL0:
	...
	jnz	short SL2		; If error, skip sending layout data


	call	MAKE_LED		; Generate actual LED byte in AL

	call	SND_DATA		; Send LED state byte

	; Update local status mirror flags safely
...
MAKE_LED:
	...
	;---------------------------------------------------------------------
	;
	mov	al, [KB_FLAG]	; Load primary status flag
	and	al, CAPS_STATE+NUM_STATE+SCROLL_STATE 
	rol	al, 4
	and	al, 07h		; Ensure absolute security mask
	retn
