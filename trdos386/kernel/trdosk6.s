...
sysent0:
	...
	; 13/01/2017 (TRDOS 386 Feaure only !)
...
fclose_0:
	...
	jns	short fclose_1 ; jump if not negative (jump if bit 7 is 0)
			; if all processes haven't closed the file, return
	;
	; eax ; First cluster
	xor	eax, eax ; 0
	mov	[ebx+OF_MODE], al ; 0 = empty entry
	;mov	[ebx+OF_STATUS], al ; 0 = empty entry
	;shl	bx, 2
	; 23/07/2022
	;shl	bl, 2
	shl	ebx, 2
	mov	[ebx+OF_FCLUSTER], eax ; 0
	mov	[ebx+OF_CCLUSTER], eax ; 0
	;mov	[ebx+OF_CCINDEX], eax ; 0
	; 23/07/2022
	;mov	[ebx+OF_OPENCOUNT], al ; 0
	mov	[u.fofp], eax ; 0
	;shr	bx, 2
	; 23/07/2022
	;shr	bl, 2
...
cnv_to_100hz_ok:
	...
	;jmp	short systime_2 ; (return 100 Hz tick count)
...
systime_4:
	...
	mov	ah, [minute]
	test	bl, 2
	jz	short systime_0	; bl = 1 (return time)
	; bl = 3
...
sysstime_7:
	cmp	bl, 4
	je	short sysstime_3 ; set system timer ticks
	cmp	bl, 5
...
sysstime_12:
	cmp	bl, 9
	ja	short sysstime_13 ; set system timer ticks for
				; 100 Hz input
