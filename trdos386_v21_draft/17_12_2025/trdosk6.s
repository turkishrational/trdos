...
; ----------------------------------------------------------------------------
; Last Update: 14/12/2025  (Previous: 27/09/2024, v2.0.9)
; ----------------------------------------------------------------------------
...
rw1:
	; 10/12/2025 - TRDOS 386 v2.0.10 (v2.1)
	; 27/09/2024
...
mget_r:
	...
	; 13/12/2025
	mov	eax, edx ; byte offset in cluster
	xor	edx, edx ; 0
	div	dword [readi.bps]
	; eax = sector index in cluster
	; edx = byte offset in sector
	mov	[readi.s_index], al ; sector index in cluster (0 to spc -1)
...
mget_r_0:
	...
	cmp	byte [esi+LD_FATType], cl ; 0
	ja	short mget_r_1
	; 13/12/2025
	movzx	eax, word [esi+LD_FS_BytesPerSec]
...
mget_r_1:
	...
	; 13/12/2025
	movzx	eax, word [esi+LD_BPB+BPB_BytsPerSec]
mget_r_2:
	mov	[readi.spc], cl  ; sectors per cluster
mget_r_2_@:
	; 13/12/2025
	mov	[readi.bps], eax	
	mul	ecx ; sectors per cluster
	mov	[readi.bpc], eax ; bytes per cluster
	; edx = 0
...
%if 0
	...
	; EAX = Logical (LBA) sector number (physical address)
	; ECX = Sector count
	; EBX = Buffer address
	; (EDX = 0)
	; ESI = Logical DOS drive description table address
...
mget_r_12:
	...
	; 01/07/2025
	; EAX = sector addr (off from start of TRFS partition)
	; EDX = LDRVT address
...
mget_w:
	...
	;    ([u.off] = file offset)
	; OUTPUTS ->
	;;   EAX = logical (physical) sector number
	;;   ESI = Logical Dos Drive Description Table address
	; 13/12/2025
	;    EAX = Physical (Logical) sector number
...
mget_w_20:
	...
	mov	eax, ebp
	; 15/12/2025
	div	ecx ; bytes per cluster
	; eax = the new last cluster index (0 based)
	mov	[writei.c_index], eax ; current cluster index
	; edx = byte offset in cluster
	mov	eax, edx
	sub	edx, edx ; 0
	div	dword [writei.bps]
	; eax = sector index
	; edx = byte offset in sector
	mov	[writei.s_index], eax
	mov	[writei.offset], edx
	...
	div	ecx ; bytes per cluster
	; eax = the new last cluster index (0 based)
	mov	[writei.l_index], eax ; last cluster index
	; edx = byte offset in cluster
	mov	eax, edx
	sub	edx, edx ; 0
	div	dword [writei.bps]
...
mget_w_21:
	; 10/12/2025 - TRDOS 386 v.2.0.10 (v2.1)
	; allocate new (and empty) cluster chain
	; but do not link/add to the last cluster of the file
	; before ensuring successful allocation

	; 13/12/2025
	;mov	[i.size], eax ; new file size
	; 14/12/2025
	mov	[writei.filesize], eax

	;sub	edx, edx ; 0
	mov	eax, [i.size] ; file size
	div	ecx
	; eax = cluster index for current end of the file

	; 15/12/2025
	mov	ebx, [writei.l_index]
		; cluster index for new end of the file
	mov	[writei.eof_c_index], eax
	sub	ebx, eax
	ja	short mget_w_38 ; not same cluster

	; 14/12/2025
	mov	eax, [writei.fclust]

	; if the first cluster is zero (here),
	;    also the file size ([i.size]) must be zero
	or	eax, eax	; is the file size ZERO ?
	jz	short mget_w_38 ; yes, newly created file

	; 14/12/2025
	mov	ecx, [writei.c_index]

	; check validation (fast operation) parameters
	cmp	esi, [writei_p_ldrvt]
	jne	short mget_w_21_@
	cmp	eax, [write_p_fclust]
	jne	short mget_w_21_@
	cmp	ecx, [write_p_c_index]
	jne	short mget_w_21_@
	mov	eax, [write_p_cluster]
	jmp	short mget_w_21_@@
...
mget_w_21_@:
	...
	; set validation (fast operation) parameters
	mov	[writei_p_ldrvt], esi
	mov	[write_p_cluster], eax
	;mov	ecx, [writei.c_index]
	mov	[write_p_c_index], edx
	;mov	edx, [writei.fclust]
	mov	[write_p_fclust], edx
mget_w_21_@@:
...
mget_w_38:
	mov	[writei.l_index], eax
	; 13/12/2025
	mov	[writei.ccount], eax
	mov	edx, esi ; LDRVT address
	xor	eax, eax ; first cluster = 0
			; needed for proper allocation
	call	ADD_NEW_CLUSTER
	jc	short mget_w_anc_err
	mov	[writei.cluster], eax
	; 13/12/2025
	mov	[writei.lclust], eax

	dec	dword [writei.l_index]
	jz	short mget_w_anc_ok

	; 12/12/2025 - TRDOS 386 v2.0.10 (v2.1)
	; clear sectors (content) of the added clusters
	; (that are not the first or the last cluster)
	; (('sysseek' will use this feature))
	; ((user can move the file pointer of the open file
	;   to a position far beyond the last/next cluster.
	;   -for open for write mode-
	;   In this case, the new file data prior to
	;   the last position will be cleared/zeroed))
mget_w_ancc:
	call	ADD_NEW_CLUSTER
	jc	short mget_w_rcc
	; 12/12/2025
	...
	call	FIGREC
	mov	[writei.drv], cl
	movzx	ebx, byte [writei.spc]
	; ebx = sectors per cluster (loop count)
mget_w_gns:
...
mget_w_gns_@:
	dec	ebx
	jz	short mget_w_ancc_@
	mov	eax, [writei.sector]
	inc	eax
	mov	cl, [writei.drv]
	jmp	short mget_w_gns
...
mget_w_ancc_@:
	...
	dec	dword [writei.l_index]
	jnz	short mget_w_ancc

mget_w_anc_ok:
	mov	ebx, [writei.cluster]
	mov	eax, [writei.fclust]
	and	eax, eax
	jnz	short mget_w_39
	mov	eax, ebx
...
mget_w_39:
	; 14/12/2025
	; validation check (for fast operation)
	cmp	edx, [writei_p_ldrvt]
	jne	short mget_w_39_@
	mov	esi, [writei_p_fclust]
	cmp	eax, esi
	jne	short mget_w_39_@
	mov	esi, [writei_p_c_index]
	cmp	esi, [writei.c_index]
	jne	short mget_w_39_@
	mov	eax, [writei.p_cluster]
	jmp	short mget_w_40
mget_w_39_@:
	push	ebx
	mov	esi, edx
	...
	; 14/12/2025
	; set validation (fast operation) parameters
	mov	[writei_p_ldrvt], edx
	mov	esi, [writei.fclust]
	mov	[writei_p_fclust], esi
	mov	esi, [glc_index] ; from 'get_last_cluster'
	mov	[writei_p_c_index], esi
	mov	[writei.p_cluster], eax

mget_w_40:
	; eax = the last cluster of the file
	; link the new cluster(s) to file
	; edx = LDRVT address
	; ebx = cluster data (next cluster num)
	call	PACK
	jc	short mget_w_rcc

	; set 'free clusters' and '1st free cluster' values
mget_w_41:
	; 13/12/2025
	mov	eax, [writei.lclust]
	...
	cmp	byte [edx+LD_FATType], 2
	jna	short mget_w_42 ; not FAT32

	; FAT32 fs
	lea	esi, [edx+LD_BPB+FAT32_FreeClusters]
	mov	[edx+LD_BPB+FAT32_FirstFreeClust], ecx
	jmp	short mget_w_43
mget_w_42:
	; FAT16 or FAT12 fs
	lea	esi, [edx+LD_BPB+FAT_FreeClusters]
	mov	[edx+LD_BPB+FAT_FirstFreeClust], ecx
mget_w_43:
	mov	ebx, [esi]
	cmp	ebx, -1 ; 0FFFFFFFFh
	jnb	short mget_w_44
	sub	ebx, [writei.ccount]
	mov	[esi], ebx

	push	eax
	movzx	eax, byte [writei.spc]
	mul	ebx
	mov	[edx+LD_FreeSectors], eax
	pop	eax

	; set validation (fast operation) parameters
	mov	[writei_p_ldrvt], edx
	mov	ecx, [writei.lclust]
	mov	[writei_p_cluster], ecx
	mov	[write_p_c_index], edx
	;mov	edx, [writei.fclust]
	mov	[write_p_fclust], edx
mget_w_21_@@:
...
mget_w_44:
	; 14/12/2025
	push	dword [writei.filesize]
	pop	dword [i.size]

	; edx = LDRVT address
	call	FIGREC

	add	eax, [writei.s_index]

	; eax = physical sector number
	;	(logical sector number for whole disk)
	;  cl = physical drive/disk number

	; edx = LDRVT address

	; [writei.lclust] = new last cluster of the file
	; [writei.fclust] = first cluster of the file
	; [i.size] = (new) file size
...
mget_w_rcc:
	push	eax ; error code
	mov	eax, [writei.cluster] ; 1st cluster added
	; edx = LDRVT address
	; eax = the 1st of added clusters
	;     (but not linked to the file's clust chain)
	call	RELEASE
	jc	short mget_w_rcc_err ; eax = error code

	; free cluster count has been set in RELEASE
	; set first free cluster

	cmp	byte [edx+LD_FATType], 2
	jna	short mget_w_rcc_1 ; not FAT32

	; FAT32 fs
	lea	esi, [edx+LD_BPB+FAT32_FirstFreeClust]
	jmp	short mget_w_rcc_2
mget_w_rcc_1:
	; FAT16 or FAT12 fs
...
; 13/12/2025 - TRDOS 386 v2.0.10 (v2.1)
%if 0
	; edx = byte offset in cluster (<= 65535)
...
mget_w_4: ; 02/11/2016
	...
	;mov	ebp, eax
	;sub	ebp, [u.count] ; current file position
	; ebp = current file position
	; edx = 0
	; ecx = bytes per cluster
	; 14/12/2025
	mov	eax, ebp
	div	ecx
	xchg	eax, [writei.c_index]
	; eax = last cluster index
...
mget_w_5_@:
	...
	; 14/12/2025
	mov	[writei.p_fclust], eax
	jmp	short mget_w_10

mget_w_6:
	; 14/12/2025
	; validation check
	cmp	esi, [writei.ldrvt]
	jne	short mget_w_5_@
	mov	ecx, [writei.fclust]
	cmp	ecx, [writei.p_fclust]
	jne	short mget_w_5_@

	;cmp	eax, [writei.fs_index] ; current cluster index (>0)
	; 14/12/2025
	cmp	eax, [writei.p_c_index] ; current cluster index (>0)
	jne	short mget_w_7
	mov	eax, [writei.cluster] ; current cluster
	;jmp	short mget_w_11
	cmp	eax, [writei.p_cluster]
	je			
...
mget_w_10:
	mov	[writei.cluster], eax
			; FDT number for Singlix File System
...
mget_w_14:
	; 13/12/2025 - temporary
	; Singlix FS
	mov	eax, ERR_DRV_NR_WRITE
		; 'drive not ready or write error !'
	stc
	retn
